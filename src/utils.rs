...
use snark_verifier_sdk::{Snark, SHPLONK, halo2::{POSEIDON_SPEC, PoseidonTranscript}, gen_pk};
use snark_verifier_sdk::halo2::{aggregation::AggregationCircuit, read_snark, gen_snark_shplonk};
...
pub fn verify(snark: &Snark) -> Result<()> {
    ...
    // dbg!(&proof);
    PlonkVerifier::verify(&vk, &protocol, &instances, &proof).map_err(|e| anyhow!("Verification Failed: {:?}", e))
}
...
pub fn aggregate(path: impl AsRef<Path>, _is_recursive: bool) -> Result<()> {
    ...
    assert!(snarks.len() > 0);
    let k = snarks.iter().map(|snark| snark.protocol.domain.k).max().unwrap();
    let params = gen_srs(k as u32);

    let agg_circuit = AggregationCircuit::keygen::<SHPLONK>(&params, snarks.clone());
    let pk = gen_pk(&params, &agg_circuit, Some(Path::new("data/agg_circuit.pkey")));
    let break_points = agg_circuit.break_points();
    let agg_circuit = AggregationCircuit::new::<SHPLONK>(
        CircuitBuilderStage::Prover,
        Some(break_points.clone()), // TODO: check meaning
